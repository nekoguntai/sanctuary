 ...
 *
 * The X-Gateway-Request header authenticates this as an internal request.
 */
async function getDevicesForUser(userId: string): Promise<DeviceInfo[]> {
  try {
    const response = await fetch(`${config.backendUrl}/api/v1/push/by-user/${userId}`, {
      headers: {
...
function connect(): void {
  ...
  ws.on('open', () => {
    ...
    // Authenticate as gateway
    ws?.send(JSON.stringify({
      type: 'gateway_auth',
      secret: config.jwtSecret, // Use JWT secret as shared secret
    }));
  });
  ...
  ws.on('message', (data) => {
    try {
      ...
      if (message.type === 'auth_success') {
        log.info('Gateway authenticated with backend');
        return;
      }
