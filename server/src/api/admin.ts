 ...
 */
router.get('/monitoring/services', authenticate, requireAdmin, async (req: Request, res: Response) => {
  try {
    ...
    log.error('[ADMIN] Get monitoring services error', { error: getErrorMessage(error) });
    res.status(500).json({
      error: 'Internal Server Error',
 ...
 */
router.put('/monitoring/services/:serviceId', authenticate, requireAdmin, async (req: Request, res: Response) => {
  try {
    ...
    if (customUrl && typeof customUrl === 'string' && customUrl.trim()) {
      ...
      log.info('[ADMIN] Monitoring service URL updated', {
        serviceId,
      ...
      log.info('[ADMIN] Monitoring service URL cleared', {
        serviceId,
        admin: req.user?.username,
      });
    }
    ...
    log.error('[ADMIN] Update monitoring service error', { error: getErrorMessage(error) });
    res.status(500).json({
      error: 'Internal Server Error',
